...
Priority triage rule — Patients with `ca ≥ 1` and elevated `oldpeak` should be escalated for cardiologist review within 7 days.
Stress-test screening protocol — Patients over 50 with below-median max heart rate under stress testing should be flagged for earlier intervention.
Recall-aware threshold policy — Tune the classification threshold to favor Recall over Precision, given the asymmetric cost of missed diagnoses.
Data governance — Exclude non-clinical identifiers from the feature set in future pipeline iterations.
